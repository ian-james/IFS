...
def getProcessInfo( cmd, outFile, errorFile ):
    ...
    with open(outFile, 'w') as fout:
        with open(errorFile,'w') as ferr:
            ...
            # Hack to get wildcard to be processed as expected
            print( args )
            print("**************")
...
def main(argv):
    ...
    options = { 'tool': 'cppcheck', 'ifs': True,  'language':'c', 'errorLevel':'all', 
                'flags': '--inconclusive', 'std': 'c99','suppress':'','dir':'', 'splitSeq': "##",
                 'splitTypes': [ "file", "lineNum", "type", "category", "feedback"],
                 'outFile':'stdout.txt', 'outErrFile':'stderr.txt',
                 'initP': "--"
                }
    ...
    if idirectory != '':
        ...
        if( cmd ):
            code, out, err = getProcessInfo( cmd, 'stdout.txt', 'stderr.txt' )
            #print ("Start")
            #print ("Code: " + str(code))
            #print ("*********************")
            #print (out)
            ...
            with open(options['outErrFile'], 'r') as errFile:
                errors = errFile.read()
