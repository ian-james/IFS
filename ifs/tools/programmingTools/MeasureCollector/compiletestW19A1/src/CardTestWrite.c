...
static SubTestRec _tWriteSubTest2(int testNum, int subTest){
    ...
    if (err == WRITE_ERROR){
        sprintf(feedback, "Subtest %d.%d: Successfully handled NULL Card object.",testNum, subTest);
        result = createSubResult(SUCCESS, feedback);
        return result;
    }
    else
    {
        sprintf(feedback, "Subtest %d.%d: Failed to handle NULL Card object.",testNum, subTest);
        result = createSubResult(FAIL, feedback);
        return result;
    }
}
...
testRec* _tWriteTestNull(int testNum){
    ...
    sprintf(feedback, "Test %d (2%%): writeCard (NULL arguments)", testNum);
...
testRec* _tValidWriteTest1(int testNum){
    return _tValidWriteTestGeneric  (testNum, minValid(), 3, "Minimum valid Card object");
}
...
testRec* _tValidWriteTest10(int testNum){
    return _tValidWriteTestGeneric  (testNum, testCardFromRFC(), 4, "Valid Card based on the sample Card from the vCard spec");
}
