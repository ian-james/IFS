...
DateTime* _tCreateInvDT(char* date, char* time, bool UTC, bool isText, char* text){
    DateTime* tempDT = malloc(sizeof(DateTime)+strlen(text)+1);
    tempDT->isText = isText;
    tempDT->UTC = UTC;
...
Property* _tCreateInvTestProp(char* propName, char* propGroup, bool hasParam, bool hasVal){
	Property* tmpProp;
