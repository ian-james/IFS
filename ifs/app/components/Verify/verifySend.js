...
var viewPath = require(__dirname + '/');
...
module.exports = {
        ...
        db.query(q, [uid, route], function(qErr, qRet) {
            if (qErr) {
                ...
                if (qRet[0]) {
                    ...
                    var insert = dbHelpers.buildInsert(dbcfg.verify)table) + dbHelpers.buildValues(['userId', 'type', 'token']);
    ...
    sendLink: function(email, link, subject, message) {
        ...
        // lookup uid
        var uid = 0;
        var lookup = dbHelpers.buildSelect(dbcfg.users_table) + dbHelpers.buildWhere(['id', 'username']);
        db.query(lookup, [uid, email]], function(err, ret) {
            if (err) {
                Logger.error(err);
                return false;
            } else
        ...
        // lookup name
        var name = '';
        var lookup = dbHelpers.buildSelect(dbcfg.students_table) + dbHelpers.buildWhere(['userId']);
        db.query(lookup, [uid], function(err, ret) {
            if (err)
                Loggger.error(err);
                return false;
            } else
